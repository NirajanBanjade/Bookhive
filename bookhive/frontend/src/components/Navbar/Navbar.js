...
const Navbar = () => {
  ...
  // open/close and lazy-load first page
  const toggleNotifications = async () => {
    const next = !openNotif;
    setOpenNotif(next);
    if (next && notifItems.length === 0) {
      await fetchPage({ page: 1, limit: 10 }); // newest-first handled on backend
    }
  };
  ...
  useEffect(() => {
    function onDocClick(e) {
      ...
      if (!notifWrapRef.current.contains(e.target)) setOpenNotif(false);
    }
    if (openNotif) document.addEventListener('mousedown', onDocClick);
